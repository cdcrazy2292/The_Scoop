...
function updateComment(url, request) {
    const id = Number(url.split('/').filter(segment => segment)[1]);
    const requestUpdateComment = request.body && request.body.comment;
    const savedComment = database.comments[id];
    const response = {};

    if (!id || !requestUpdateComment || !requestUpdateComment.body) {
        response.status = 400;
    } else if (!savedComment) {
        response.status = 404;
    } else {
        database.comments[id].body = requestUpdateComment.body;
...
function deleteComment(url, request) {
    ...
    if (savedComment) {
        ...
        const userComment = database.users[savedComment.username].commentIds;
        const articles = database.articles;
        userComment.splice(userComment.indexOf(id), 1);

        //Using lodash to iterate over Collection
        lodash.forEach(articles, each => {
            let commentIds = each.commentIds;
...
function loadDatabase() {
    let jsonDatabase = {};
    try {
        let doc = yaml.safeLoad(fs.readFileSync('database.yaml', 'utf8'));
        // console.log('Doc', doc);
        jsonDatabase = doc;
    } catch (e) {
        console.log(e);
...
function saveDatabase(dbData) {
    let jsonDatabase = {};
    jsonDatabase = yaml.safeDump(dbData);
    fs.truncate('database.yaml', 0);
    fs.writeFile('database.yaml', jsonDatabase);
}
